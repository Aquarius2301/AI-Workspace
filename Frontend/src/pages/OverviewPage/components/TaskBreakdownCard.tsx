...
const TASK_BADGE_CONFIG = [
  { key: "toDo" as const, color: "#1677ff" },
  { key: "doing" as const, color: "#fa8c16" },
...
export function TaskBreakdownCard({
  ...
  return (
    <AICard isLoading={isLoading} title={t("overviewPage.taskStats.title")}>
      <Flex vertical gap={20}>
        <Progress
          ...
          format={() =>
            taskSummary.total > 0
              ? `${taskSummary.done}/${taskSummary.total} ${t("overviewPage.taskStats.done").toLowerCase()}`
              : "—"
          }
        />
        <Row gutter={[8, 12]}>
          {TASK_BADGE_CONFIG.map((item) => (
            <Col xs={12} sm={8} md={4} key={item.key}>
              <Flex align="center" gap={8}>
                ...
                >
                  {t(`overviewPage.taskStats.${item.key}`)}
                </Text>
