...
public sealed record MeResult(string Avatar, string Name, string Email, LanguageDisplay Language);
...
public sealed class MeQueryHandler : IRequestHandler<MeQuery, MeResult>
{
    ...
    public async Task<MeResult> Handle(MeQuery request, CancellationToken cancellationToken)
    {
        ...
        return new MeResult(user.AvatarUrl ?? "", user.Name, user.Email, user.Language);
    }
}
