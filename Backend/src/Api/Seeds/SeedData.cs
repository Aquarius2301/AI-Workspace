...
namespace AIWorkspace.Api.Seeds;
...
public static class SeedData
{
    public static async Task Initialize(IServiceProvider serviceProvider)
    {
        ...
        Console.WriteLine("This will DELETE all existing data and re-seed with sample data. ");
        Console.WriteLine("This action CANNOT be undone.");
        Console.ResetColor();
        Console.Write("Press Enter to continue, or type 'no' to skip: ");
        var inputLine = await Console.In.ReadLineAsync();
        var input = inputLine?.Trim().ToLowerInvariant();
        if (input == "no" || input == "n")
        {
        ...
        // Users (10 users)
        // ---------------------------------------------------------------
        var passwordHash = PasswordHasherHelper.Hash("Password123@");
        ...
        {
            // Alpha Team: Khang (Admin), Minh (CoAdmin), Thao (Member), Hoang (Member), Bao (Member)
            new()
            {
            ...
            // Beta Team: Alex (Admin), Linh (CoAdmin), Khang (Member), Thao (Member), Chi (Member)
            new()
            {
            ...
            // Gamma Team: Huy (Admin), Alex (CoAdmin), Khang (Member), Minh (Member), Bao (Member)
            new()
            {
            ...
            // Delta Team: Mai (Admin), Linh (CoAdmin), Thao (Member), Hoang (Member), Chi (Member)
            new()
            {
        ...
        Console.WriteLine(
            "10 users, 4 teams, 20 members, 8 projects, 24 project members, 48 tasks seeded successfully!"
        );
        Console.ResetColor();
    }
}
